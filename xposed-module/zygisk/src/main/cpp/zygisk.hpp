...
// Canonical source: https://github.com/topjohnwu/zygisk-module-sample/blob/master/module/jni/zygisk.hpp
// Vendored verbatim (Zygisk API v4). ZygiskNext / ReZygisk implement this same ABI, so the module is
// portable across Zygisk providers. Do not edit — it is an external contract.
#pragma once
...
Zygote is the parent of all app processes; it preloads classes/resources, and is where Zygisk lives.

A Zygisk module is a shared library that gets loaded into the zygote process. It can then hook into
the app specialization process to run its own code in the target app's process.

****************************
* Development Environment
****************************

Refer to the sample module's README for how to set up a build environment.
...
Please note that modules are unloaded after post[XXX]Specialize is called, so do NOT
keep any global reference to the API handle.
...
public:
    ...
    //
    // If you need to run some operations as superuser, you can call Api::connectCompanion() to get
    // a socket to do IPC calls with a root companion process.
...
enum Option : int {
    ...
    // The unmounting happens during app process specialization.
    //
    // Set this option to force all Magisk and modules' files to be unmounted from the
    // mount namespace of the process. This may be beneficial for a module that would like
    // to hide traces of Magisk from the process, e.g. anti-detection modules.
...
inline bool Api::exemptFd(int fd) {
    return tbl->exemptFd ? tbl->exemptFd(fd) : false;
}
...
inline bool Api::pltHookCommit() {
    return tbl->pltHookCommit ? tbl->pltHookCommit() : false;
}
