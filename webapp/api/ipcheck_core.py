...
/* Sub-lines are short by nature ("1128 total", "low"). The cap is a backstop so a long one can never
   widen its column the way the verdict reason did before it moved to the pill's title. */
table.bulk td .sub{max-width:120px;overflow:hidden;text-overflow:ellipsis}
...
const vpill=(v,why)=>`<span class="vpill v-${v||'unknown'}-p"`+
  (why?` title="${esc(why)}"`:'')+`>${esc((v||'—').toUpperCase())}</span>`;
...
$('#bulkgo').onclick=async()=>{
  ...
  const COLS=[
    ...
    {k:'verdict', h:'Verdict',
     ...
     cell:x=>{if(x.busy)return '<span class="vpill v-unknown-p">…</span>';
       // The reason goes on the PILL, not under it. Printed inline it was prose in a scanning table:
       // it made Verdict the second-widest column, ellipsised to unreadable stubs ("no proxy fla…"),
       // and for a CLEAN row it was a double negative that duplicated the Flags column next to it
       // ("no proxy flags" beside a Flags cell already reading "none"). The full text is one hover
       // away and spelled out in the row's own BECAUSE line.
       const why=x.r.error||(x.r.verdict_factors||[]).join(' · ')||x.r.verdict_reason||'';
       return vpill(x.r.error?'unknown':x.r.verdict, why);}},
    ...
    {k:'gii', h:'GII', ttl:'getIPIntel proxy/hosting probability, 0-1', drop:1,
     ...
     cell:x=>x.r&&x.r.getipintel_score!=null
       ...
       :`<span class="dim nodata" title="${esc(giiWhy(x.r))}">n/a</span>`},
    {k:'abuse', h:'AbuseDB', ttl:'AbuseIPDB confidence — reports in the last 90 days', drop:1,
    ...
    {k:'scam', h:'SCAM', ttl:'Scamalytics score — shown, not scored: it tracks the ISP score, not this IP', drop:1,
     ...
     cell:x=>x.r&&x.r.scam_risk
       ...
       :'<span class="dim nodata" title="No Scamalytics credentials — not measured">n/k</span>'},
    {k:'type', h:'Exit', ttl:'Exit type — hosting, Tor or a mobile line, where it could be determined', drop:1,
     ...
     cell:x=>x.r&&x.r.connection_class
       ...
       :`<span class="dim nodata" title="${x.r&&x.r.scam_risk!=null
  ...
  const draw=()=>{
    ...
    // A 15-column table that spends width on a column every row left empty is width taken from the
    // columns carrying the answer. A residential exit has no datacenter/Tor/mobile signal at all, and
    // getIPIntel rate-limits itself away partway through a bulk run — so those two are routinely dashes
    ...
    let h=`<div class=bulkwide><div class="blk panel"><div class=bsum>`+
      ...
      SHOWN.map(c=>`<th class=s data-sort="${c.k}"`+
    ...
    for(const x of view){
      h+=`<tr class="${x.open?'open':''}"><td class=cw><button class=chev data-deep="${x.i}" `+
        ...
        SHOWN.map(c=>`<td${c.k==='isp'||c.k==='loc'?' class=cap':''}>`+
          ((x.busy&&c.k!=='proxy'&&c.k!=='status')?'<span class=dim>·</span>':c.cell(x))+
          `</td>`).join('')+`</tr>`;
      if(x.open)h+=`<tr class=detrow><td colspan="${SHOWN.length+1}">${bulkDetail(x)}</td></tr>`;
    }
    h+=`</tbody></table></div><p class=note style="padding:10px 2px 0">`+
      ...
      (dropped.length?` · hidden because no row had one: ${esc(dropped.map(c=>c.h).join(', '))}`:'')+
      `</p></div></div>`;
    out.innerHTML=h;
  };
...
_SIGNAL_CODES_JSON = json.dumps({label: code for _key, label, code in IPQS_FLAGS},
                                ensure_ascii=True, sort_keys=True)
# Valid JSON is NOT automatically safe inside a <script>: a `</script>` sequence inside a string ends
# the ELEMENT, and everything after it is parsed as markup. IPQS_FLAGS is ours today, but the mechanism
# has to be safe for the day it is not. Escaping these three is the standard, lossless fix — a JSON
# parser reads them back as the original characters. ensure_ascii keeps every other byte out of it.
for _ch, _esc in (("<", r"\u003c"), (">", r"\u003e"), ("&", r"\u0026")):
    _SIGNAL_CODES_JSON = _SIGNAL_CODES_JSON.replace(_ch, _esc)
PAGE = PAGE.replace("__SIGNAL_CODES__", _SIGNAL_CODES_JSON)
