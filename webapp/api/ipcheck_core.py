...
<div class=wrap>
  ...
  <div class=panel>
    ...
    <details style="margin-top:14px"><summary>Bulk: compare many proxies</summary>
      <textarea id=bulk rows=5 placeholder="one proxy per line — host:port:user:pass (or host:port)"></textarea>
      <button class=go id=bulkgo style="margin-top:10px">Check all</button>
      <p class=note>One line = one proxy. Checked through the transport above. Results in one copyable table.</p>
    </details>
    <p class=note style="margin-top:10px"><b>Works with no keys</b> — datacenter + 17 blocklists + getIPIntel grade every exit.<br>Add keys above to sharpen it.</p>
  </div>
  <div id=out></div>
</div>
<script>
...
async function boot(){
  if(!$('#ip').value && !$('#proxy').value){
    try{const o=await (await fetch('https://ipwho.is/')).json(); if(o&&o.ip)$('#ip').value=o.ip;}catch(e){}
  }
  $('#go').click();
}
...
out.addEventListener('click',async e=>{
  ...
  const o=b.textContent; b.textContent=b.classList.contains('cc')?'✓':'Copied ✓'; b.classList.add('done');
...
// Bulk: check many proxies (one per line) and show them in one sortable, copyable comparison table.
...
$('#bulkgo').onclick=async()=>{
  ...
  const draw=()=>{
    ...
    let h=`<div class="blk panel tablewrap"><table class=bulk><thead><tr><th>Proxy</th><th>Exit IP</th>`+
      `<th>Verdict</th><th>getIPIntel</th><th>Lists</th><th>Type</th><th>Copy</th></tr></thead><tbody>`;
    for(const x of rows){const r=x.r||{};
      ...
      const type=r.connection_class||(r.ip?'residential':(x.busy?'':'—'));
      const vd=x.busy?'<span class="vpill v-unknown-p">…</span>':vpill(r.error?'unknown':r.verdict);
      h+=`<tr>${cell(x.line,'pxcell')}${cell(ip)}<td>${vd}</td>${cell(g)}${cell(hits)}${cell(type)}`+
        `<td style="white-space:nowrap"><button class=cc data-copy="${esc(x.line)}">proxy</button> `+
        (r.ip?`<button class=cc data-copy="${esc(r.ip)}">ip</button>`:'')+`</td></tr>`;
    }
    h+=`</tbody></table><p class=note style="padding:10px 10px 0">${done}/${rows.length} checked · click Proxy or IP to copy</p></div>`;
    out.innerHTML=h;
  };
  draw();
