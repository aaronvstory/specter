...
$('#bulkgo').onclick=async()=>{
  ...
  const COLS=[
    ...
    {k:'fraud', h:'IPQS', ttl:'IPQualityScore fraud score, 0-100', get:x=>num(x.r&&x.r.fraud_score), blank:x=>!x.r||x.r.fraud_score==null,
    ...
    {k:'gii', h:'GII', ttl:'getIPIntel proxy/hosting probability, 0-1', blank:x=>!x.r||x.r.getipintel_score==null,
    ...
    {k:'abuse', h:'AbuseDB', ttl:'AbuseIPDB confidence — reports in the last 90 days', blank:x=>!x.r||x.r.abuse_confidence==null,
    ...
    {k:'scam', h:'SCAM', ttl:'Scamalytics score — shown, not scored: it tracks the ISP score, not this IP', blank:x=>!x.r||x.r.scam_score==null,
    ...
    {k:'type', h:'Exit', ttl:'Exit type — hosting, Tor or a mobile line, where it could be determined', blank:x=>!x.r||!x.r.connection_class,
  ...
  const draw=()=>{
    ...
    const dropped=settled?COLS.filter(c=>c.blank&&rows.every(c.blank)):[];
