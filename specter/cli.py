...
def cmd_new(a):
    store = _store()
    p = P.generate_unique(store, us_bias=not a.no_us_bias, country=getattr(a, "country", "US"))
    store.save()
    _save(ACTIVE, p)
    if a.name:
        vault = _load(VAULT, {})
...
def build_parser():
    ...
    p = sub.add_parser("new"); p.add_argument("--name"); p.add_argument("--no-us-bias", action="store_true"); p.add_argument("--country", default="US"); p.set_defaults(f=cmd_new)
    ...
    p = sub.add_parser("rotate"); p.add_argument("--pkg", default="com.liuzh.deviceinfo"); p.add_argument("--no-clear", action="store_true"); p.add_argument("--no-us-bias", action="store_true"); p.add_argument("--country", default="US"); p.set_defaults(f=cmd_rotate)
