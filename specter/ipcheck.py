...
<div class=wrap>
  ...
  <div class=panel>
    ...
    <details><summary>Bulk — compare many proxies</summary>
      <textarea id=bulk rows=5 placeholder="one proxy per line"></textarea>
      <button class=go id=bulkgo style="margin-top:10px">Check all</button>
      <div class="rows hint">
        <div class=rw><i>Input</i><div>one proxy per line, any format above</div></div>
        <div class=rw><i>Transport</i><div>the selector above</div></div>
      </div>
    </details>
...
async function boot(){
  ...
  for(const [url,pick] of [['https://ipwho.is/',o=>o&&o.success!==false&&o.ip],
    ...
    try{const ip=pick(await (await fetch(url)).json()); if(ip){$('#ip').value=ip; return;}}catch(e){}
  }
}
...
out.addEventListener('click',async e=>{
  const b=e.target.closest('.copy,.cc'); if(!b)return;
...
let bulk=null;
out.addEventListener('click',e=>{const b=e.target.closest('[data-deep]'); if(!b||!bulk)return;
  const x=bulk.rows[+b.dataset.deep]; if(!x)return; x.open=!x.open; bulk.draw();});
$('#bulkgo').onclick=async()=>{
  const lines=$('#bulk').value.split('\n').map(s=>s.trim()).filter(Boolean);
  if(!lines.length)return;
  saveKeys();
  const b=$('#bulkgo'); b.disabled=true; b.textContent='Checking…';
  const rows=lines.map(line=>({line, r:null, busy:true}));
  const cell=(txt,cls)=>`<td class="${cls||''}">${esc(txt)}</td>`;
  const draw=()=>{
    const done=rows.filter(x=>!x.busy).length;
    let h=`<div class="blk panel tablewrap"><table class=bulk><thead><tr><th>Proxy</th><th>Alive</th>`+
      `<th>Latency</th><th>Exit IP</th>`+
      `<th>Verdict</th><th>getIPIntel</th><th>Lists</th><th>Type</th><th>Copy</th></tr></thead><tbody>`;
    rows.forEach((x,i)=>{const r=x.r||{};
      const ip=r.ip||(x.busy?'…':'—');
      const g=r.getipintel_score!=null?r.getipintel_score.toFixed(2):'—';
      const hits=r.ip?String((r.blacklists||[]).length):(x.busy?'':'—');
      // Never invent "residential" — connection_class returns null when the name heuristic can't tell.
      const type=r.connection_class||(x.busy?'':'—');
      // Dead-or-alive is the first thing you want when comparing a list, and latency the second — neither
      // is a reputation signal, so no source reports them.
      const alive=x.busy?'<span class="vpill v-unknown-p">…</span>'
        :r.proxy_alive===false||r.error?'<span class="vpill v-dirty-p">DEAD</span>'
        :r.ip?'<span class="vpill v-clean-p">UP</span>':'<span class="vpill v-unknown-p">—</span>';
      const ms=r.proxy_ms!=null?r.proxy_ms+' ms':(x.busy?'':'—');
      const vd=x.busy?'<span class="vpill v-unknown-p">…</span>':vpill(r.error?'unknown':r.verdict);
      h+=`<tr>${cell(x.line,'pxcell')}<td>${alive}</td>${cell(ms)}${cell(ip)}<td>${vd}</td>${cell(g)}${cell(hits)}${cell(type)}`+
        `<td style="white-space:nowrap"><button class=cc data-copy="${esc(x.line)}">proxy</button> `+
        (r.ip?`<button class=cc data-copy="${esc(r.ip)}">ip</button> `:'')+
        (r.ip?`<button class=cc data-deep="${i}">${x.open?'hide':'why'}</button>`:'')+`</td></tr>`;
      // Same per-source breakdown as the single check, inline under its row — a bulk verdict is only
      // actionable if you can see what produced it.
      if(x.open&&r.ip)h+=`<tr><td colspan=9 style="padding:0 0 12px">${deep(r)||'<p class=note>no source detail</p>'}</td></tr>`;
    });
    h+=`</tbody></table><p class=note style="padding:10px 10px 0">${done}/${rows.length} checked · Proxy/IP copies · Why opens the breakdown</p></div>`;
    out.innerHTML=h;
  };
  bulk={rows, draw};      // hand the current run to the once-registered row expander below
  draw();
  let idx=0; const CAP=4;
  const worker=async()=>{ while(idx<rows.length){const x=rows[idx++];
    try{const resp=await fetch(API,{method:'POST',body:checkBody(x.line)}); x.r=await resp.json();}
